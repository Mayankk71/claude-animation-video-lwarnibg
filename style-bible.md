...
- "(prompt-craft note, not a visual trait — supersedes any earlier, softer version of this rule) when reference images are attached, do not describe character appearance in the prompt text AT ALL, not even briefly by name/role, and state the 'reference images are attached' instruction only ONCE per prompt set, never repeated inside every individual shot — inside each shot's actual generation text, use nothing more than a bare character label. Real test generations kept drifting from the reference images through two earlier, progressively-trimmed attempts at this rule; repeating any instructional language about the images inside every shot's generation text is itself the kind of clutter that competes with image conditioning"
